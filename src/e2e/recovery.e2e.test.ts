...
test('F3: a recovered run can still be rejected at the plan gate (cancels the run)', { skip: e2eSkip }, async () => {
  ...
  await waitState(h.api, crashed.planReject); // workflow returns after cancelRun
  const detail = await h.api.getRun({ runId: crashed.planReject });
  assert.equal(detail.run.status, 'cancelled'); // plan-gate reject cancels (B3), even post-recovery
});
