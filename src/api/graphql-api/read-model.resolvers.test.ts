...
test('read-model resolvers delegate to domain api services', async () => {
  ...
  assert.equal(new RunsResolver(runsApi as never).simulateRoute({ title: 'Build', pipelineId: 'local-change' }), 'route');
  ...
  assert.equal(new MethodResolver(methodApi as never).runProfile({ pipelineId: 'local-change', profileId: 'custom-exact' } as never), 'profile');
  ...
  assert.equal(new MethodResolver(methodApi as never).createRunProfile({ pipelineId: 'local-change', profileId: 'custom-exact', displayName: 'Custom', profile: {} } as never), 'createProfile');
  assert.equal(new MethodResolver(methodApi as never).updateRunProfile({ pipelineId: 'local-change', profileId: 'custom-exact', expectedProfileRevisionHash: 'hash' } as never), 'updateProfile');
  assert.equal(new MethodResolver(methodApi as never).deprecateRunProfile({ pipelineId: 'local-change', profileId: 'custom-exact', expectedProfileRevisionHash: 'hash' } as never), 'deprecateProfile');
  ...
  assert.ok(calls.some((call) => call === 'profile:{"pipelineId":"local-change","profileId":"custom-exact"}'));
  assert.ok(calls.some((call) => call === 'validateProfile:{"pipelineId":"local-change","profile":{}}'));
  assert.ok(calls.some((call) => call === 'createProfile:{"pipelineId":"local-change","profileId":"custom-exact","displayName":"Custom","profile":{}}'));
  assert.ok(calls.some((call) => call === 'updateProfile:{"pipelineId":"local-change","profileId":"custom-exact","expectedProfileRevisionHash":"hash"}'));
  assert.ok(calls.some((call) => call === 'deprecateProfile:{"pipelineId":"local-change","profileId":"custom-exact","expectedProfileRevisionHash":"hash"}'));
