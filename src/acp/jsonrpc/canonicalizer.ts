...
export function canonicalizeJsonRpcValue(value: unknown): JsonRpcValue | undefined {
  ...
  const stack: CanonicalizationFrame[] = [{ kind: 'enter', value, depth: 1 }];
  ...
  try {
    while (stack.length > 0) {
      ...
      const classified = classifyJsonValueKind(frame.value);
      if (classified.kind === 'invalid') return undefined;
      if (classified.kind === 'primitive') {
        assignCanonicalValueToParent(frame, classified.value, state);
        continue;
      }
      const entries = readJsonContainerEntries(classified.value);
      if (!entries || ancestors.has(classified.value)) return undefined;
      const canonical = createEmptyCanonicalContainer(entries);
      assignCanonicalValueToParent(frame, canonical, state);
      ancestors.add(classified.value);
      stack.push({ kind: 'leave', value: classified.value });
      queueCanonicalizationEntries(stack, entries.entries, frame.depth, canonical);
    }
  } catch {
    return undefined;
  }
...
type CanonicalizationFrame = EnterFrame | { kind: 'leave'; value: object };
...
function readJsonContainerEntries(value: object): JsonContainerEntries | undefined {
  if (Array.isArray(value)) return readArrayEntries(value);
  return isPlainRecord(value) ? readObjectEntries(value) : undefined;
}

function readArrayEntries(value: unknown[]): JsonContainerEntries | undefined {
  if (Object.getPrototypeOf(value) !== Array.prototype) return undefined;
  const length = readArrayLength(value);
  if (length === undefined || !hasCompleteArrayIndices(value, length)) return undefined;
...
function hasCompleteArrayIndices(value: unknown[], length: number): boolean {
  let ownIndexKeyCount = 0;
  for (const key of Reflect.ownKeys(value)) {
    const index = parseArrayIndexKey(value, key, length);
...
function parseArrayIndexKey(value: unknown[], key: PropertyKey, length: number): number | null | undefined {
...
function readObjectEntries(value: Record<string, unknown>): JsonContainerEntries | undefined {
  const entries: JsonContainerEntries['entries'] = [];
  for (const key of Reflect.ownKeys(value)) {
    if (typeof key !== 'string') return undefined;
    const descriptor = Object.getOwnPropertyDescriptor(value, key);
...
function classifyJsonValueKind(value: unknown): JsonValueKind {
...
function assignCanonicalValueToParent(
  frame: EnterFrame,
  value: JsonRpcValue,
...
function createEmptyCanonicalContainer(entries: JsonContainerEntries): JsonContainer {
  return entries.kind === 'array'
    ? isolateInheritedRuntimeHooks<JsonRpcValue[]>([])
    : isolateInheritedRuntimeHooks<{ [key: string]: JsonRpcValue }>({});
}

function queueCanonicalizationEntries(
  stack: CanonicalizationFrame[],
