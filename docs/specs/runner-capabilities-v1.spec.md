...
| `authMode` | enum `cli-session`\|`api-key`\|`gateway-token`\|`none` | How the runner authenticates. Used by selection, host doctor, and explicit plan-compilation availability checks; it does not imply workspace policy. |
