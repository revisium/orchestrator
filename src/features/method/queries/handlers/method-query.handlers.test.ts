...
test('method query handlers delegate and normalize method records', async () => {
  ...
  const api = {
    async listRoles() {
      return [{ id: 'role_1', name: 'developer', surface: '', rights: '', playbookId: 'playbook_1', playbookRoleId: 'developer' }];
    },
    async getRole(id: string) {
      assert.equal(id, 'developer');
      return { name: 'developer', systemPrompt: 'Build', allowedTools: [], scopeRules: {} };
    },
    async listPlaybooks() {
