...
import {
  ...
  parseAcpPeerRequest,
} from "../protocol/parsing/index.js";
...
export class AcpInvocation {
  constructor(
    @Inject(AcpSession) private readonly session: AcpSession,
    @Inject(AcpPermissionRequestHandler)
    private readonly permissionHandler: AcpPermissionRequestHandler,
    @Inject(AcpPromptOutcomeCollector)
    private readonly outcomeCollector: AcpPromptOutcomeCollector,
  ) {}
  ...
  run(
    ...
    return runAcpInvocationWithComponents(request, deps, {
      ...
      outcomeCollector: this.outcomeCollector,
    });
  }
}
