...
test('updateRow throws HTTP_ERROR when SDK success response has no row', () => {
  assert.throws(
    ...
    (error: unknown) =>
      error instanceof ControlPlaneError && error.code === 'HTTP_ERROR',
  );
});
...
test('withRequestTimeout supplies an abort signal (and combines with a caller signal)', async () => {
  ...
  assert.ok(
    seen[0] instanceof AbortSignal,
    'adds a timeout signal when the caller supplies none',
  );
  ...
  assert.ok(
    seen[1] instanceof AbortSignal,
    'still supplies a (combined) signal when the caller has one',
  );
});
...
test('withRequestTimeout aborts a request that outlives the timeout', async () => {
  ...
  await assert.rejects(() =>
    withRequestTimeout(hangingFetch, 25)('http://example.test'),
  );
});
