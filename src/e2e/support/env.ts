...
export function dbosDbNameForFile(filePath: string): string {
  ...
  const identity = e2eIndex >= 0
    ? normalized.slice(e2eIndex + 1).join('_')
    : basename(filePath);
  const base = identity
    .replace(/\.e2e\.test\.ts$/, '')
    .replace(/\.(m|c)?ts$/, '')
 ...
 * `src/e2e/support/fixtures/playbook` (roles + pipelines catalog), so e2e runs are stable and independent
